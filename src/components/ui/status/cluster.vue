...
<template>
  <div v-if="!status" class="text-[14px]">
    <template v-if="stackType === 0">
      <span v-if="isCreated" class="flex_c gap-1.5"> <IconLoadingLoop style="color: #0065fd" /> Creating </span>
      <span v-if="isSuccess" class="flex_c gap-1.5">
        <CheckCircleOutlined style="color: #16a34a" />
        Running
      </span>
      <span v-if="isError" class="flex_c gap-1.5">
    ...
    <template v-if="stackType === 10">
      <span v-if="isCreated" class="flex_c gap-1.5"> <IconLoadingLoop style="color: #0065fd" /> scaling </span>
      <span v-if="isSuccess" class="flex_c gap-1.5">
        <CheckCircleOutlined style="color: #16a34a" />
        Running
      </span>
    ...
    <template v-if="stackType === -1">
      <span v-if="isCreated" class="flex_c gap-1.5 text-[#f16538]">
  ...
  <div v-else class="flex items-center gap-2">
    <span v-if="status === 0" class="flex_c gap-1.5"> <IconLoadingLoop style="color: #0065fd" /> Creating </span>

    <span v-if="status === 1" class="flex_c gap-1.5">
      <CheckCircleOutlined style="color: #16a34a" />
      Running
    </span>

    <span v-if="status === 20" class="flex_c gap-1.5">
      <ExclamationCircleFilled style="color: #ff4d4f" />
      Limited
    </span>
    ...
    <span v-if="status === -10" class="flex_c gap-1.5">
      <ExclamationCircleFilled style="color: #f16538" />
      Terminated
    </span>

    <span v-if="status === 100" class="flex_c gap-1.5">
      <ExclamationCircleFilled style="color: #ff4d4f" />
      Reject
    </span>
  </div>
