...
import { ref } from 'vue'
import { ITimezoneOption, useTimezoneSelect } from 'vue-timezone-select'
...
const otherInfo = ref({})
const otherLabelStyle = ref('original') // original abbrev altName
const { timezoneOptions: otherOptions, getSelectedInfo } = useTimezoneSelect({
  labelStyle: otherLabelStyle,
  timezones,
})
function otherChangeHandle(val: any) {
  otherInfo.value = getSelectedInfo(val, otherOptions)
}
</script>

<template>
  <div v-if="false" style="width: 500px">
    <div style="height: 20px; padding: 10px 0">选中值：{{ localValue }}</div>
    <a-select v-model="otherValue" style="width: 100%" filterable @change="otherChangeHandle">
  ...
  <div>
    <a-select v-model="otherValue" filterable>
