...
<template>
  <a-modal v-model:open="visible" v-bind="$bind" @ok="onOk" @cancel="close">
    <a-form ref="formRef" :rules="rules" :model="formState" autocomplete="off" layout="vertical" class="my-2">
      ...
      <div class="flex gap-4">
        <a-form-item label="Project" name="projectName" class="flex-1">
          <a-select v-model:value="formState.projectName" size="large">
            <a-select-option
              v-for="item in ['default project', 'dev project', 'web project']"
              :key="item"
              :value="item"
